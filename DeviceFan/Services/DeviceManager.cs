...
namespace DeviceFan.Services
{
    internal class DeviceManager
    {
        ...
        public async void Start()
        {
            ...
            Task.WhenAll(SetTelemetryIntervalAsync(), NetworkManager.CheckConnectivityAsync(), SendTelemetryAsync());
        }

        private async Task SetTelemetryIntervalAsync()
        {
            var _telemetryInterval = await DeviceTwinManager.GetDesiredTwinPropertyAsync(Configuration.DeviceClient, "telemetryInterval");
